...
class Sketch2Face1GANModel(BaseModel):
    ...
    def backward_G(self):
        ...
        # First, G(A) should fake the discriminator with the intermediate image
        fake_AB = torch.cat((self.real_A, self.fake_B), 1)
        pred_fake = self.netD(fake_AB)
        self.loss_G_GAN = self.criterionGAN(pred_fake, True)
        # Second, G(A) = B
        self.loss_G_L1 = self.criterionL1(self.fake_B, self.real_B) * self.opt.lambda_L1
        # Do iterative refinement; stop backprop to first generator by detaching fake_B_2
