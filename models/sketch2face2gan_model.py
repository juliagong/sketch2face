...
class Sketch2Face2GANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--discriminator_norm', type=str, default='batch', help='norm for discriminator. options: [instance | batch | spectral | none]')

        return parser

    def __init__(self, opt):
        """Initialize the sketch2face class.
        ...
        self.loss_names = ['G_GAN', 'G_2_GAN', 'G_L1', 'G_2_L1', 'D_real', 'D_fake']
        # specify the images you want to save/display. The training/test scripts will call <BaseModel.get_current_visuals>
        self.visual_names = ['real_A', 'fake_B', 'fake_B_2', 'real_B']
        # specify the models you want to save to the disk. The training/test scripts will call <BaseModel.save_networks> and <BaseModel.load_networks>
        if self.isTrain:
            self.model_names = ['G', 'G_2', 'D']
        else:  # during test time, only load G and G_2
            self.model_names = ['G', 'G_2']
        # define networks (both generator and discriminator)
        self.netG = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)

        self.netG_2 = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    def forward(self):
        ...
        self.fake_B_2 = self.netG_2(self.fake_B) # iterative refinement #1

    def backward_D(self):
        """Calculate GAN loss for the discriminator"""
        # Fake; stop backprop to the generator by detaching fake_B
        fake_AB = torch.cat((self.real_A, self.fake_B_2), 1)  # we use conditional GANs; we need to feed both input and output to the discriminator
    ...
    def backward_G(self):
        ...
        self.loss_G_2_GAN = self.criterionGAN(pred_fake_2, True)
        # Also fake discriminator with intermediate image
        ...
        self.loss_G_2_L1 = self.criterionL1(self.fake_B_2, self.real_B) * self.opt.lambda_L1
        # combine loss and calculate gradients
        self.loss_G = self.loss_G_GAN + self.loss_G_2_GAN + self.loss_G_L1 + self.loss_G_2_L1
    ...
    def optimize_parameters(self):
        ...
        self.optimizer_G.step()             # update G's weights
        self.optimizer_G_2.step()           # update G's weights
