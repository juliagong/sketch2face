...
We can specify '--dataset_mode facesg' to use this dataset.
...
class FacesGDataset(BaseDataset):
    ...
    def make_cuhk_pairs_dataset(dir, max_dataset_size=float("inf")):
        image_pairs = [] # actually (sketch, image, grayscale) triples
            ...
            for fname in fnames:
                ...
                if match:
                    ...
                    if key in pic_paths:
                        ...
                        gray_path = pic_path[:-4] + '_gray.jpg'
                        image_pairs.append((path, pic_path, gray_path))
                        del pic_paths[key]
                        continue
                ...
                if match:
                    ...
                    if key in sketch_paths:
                        ...
                        gray_path = path[:-4] + '_gray.jpg'
                        image_pairs.append((sketch_path, path, gray_path))
        ...
        for root, dirs, files in sorted(os.walk(dir)):
            for f in files:
                ...
                if 'sketch' in path: # sketch
                    if key in pic_paths:
                        pic_path = pic_paths[key]
                        gray_path = os.path.join(root[:-7], 'grayscale', pic_path.split('/')[-1])
                        image_pairs.append((path, pic_path, gray_path))
                        del pic_paths[key]
                        continue
                    sketch_paths[key] = path
                    ...
                    if key in sketch_paths:
                        sketch_path = sketch_paths[key]
                        gray_path = os.path.join(root[:-6], 'grayscale', f)
                        image_pairs.append((sketch_path, path, gray_path))
                        del sketch_paths[key]
                        continue
                    pic_paths[key] = path
    ...
    def __init__(self, opt):
        ...
        self.image_pair_paths = sorted(FacesGDataset.make_cuhk_pairs_dataset(self.dir_cuhk_images, opt.max_dataset_size) +
                FacesGDataset.make_iiitd_pairs_dataset(self.dir_iiitd_images, opt.max_dataset_size) +
                FacesGDataset.make_colorferet_pairs_dataset(self.dir_colorferet_images, opt.max_dataset_size))
    ...
    def __getitem__(self, index):
        ...
        """
        sketch_path, photo_path, gray_path = self.image_pair_paths[index]    # supposed to be strings
        ...
        return {'sketch': sketch, 'photo': photo, 'gray': gray, 'sketch_path': sketch_path, 'photo_path': photo_path, 'gray_path': gray_path}
